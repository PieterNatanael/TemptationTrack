...
struct ContentView: View {
    ...
    var body: some View {
        ...
        let currentYearCount = countInstances(for: .currentYear)
            ...
            if isShowingSplash {
                ...
                List {
                    Section(header: VStack(alignment: .leading) {
                        ...
                        Text("Total instances this month: \(previousMonthCount)")
                            .font(.title3)
                        Text("Total instances this year: \(currentYearCount)")
                            .font(.title3)
                        Text("Total instances last year: \(previousYearCount)")
                            .font(.title3)
    ...
    private func countInstances(for timeFrame: TimeFrame) -> Int {
        ...
        case .previousMonth:
            ...
            startDate = calendar.date(byAdding: .month, value: -1, to: startOfCurrentMonth)!
        case .previousYear:
            let startOfCurrentYear = calendar.date(from: calendar.dateComponents([.year], from: currentDate))!
            startDate = calendar.date(byAdding: .year, value: -1, to: startOfCurrentYear)!
            endDate = startOfCurrentYear // Ensure it doesn't include the current year
        case .currentYear:
...
//udah bagus namun ada kendala di perhitungan tahun, yang tahun lalu kebawa ke tahun ini.
